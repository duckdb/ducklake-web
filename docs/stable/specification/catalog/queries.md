...
Before anything else we need to find a snapshot ID to be queries. There can be many snapshots in the [`ducklake_snapshot` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot). A snapshot ID is a continously increasing number that identifies a snapshot. In most cases, you would query the most recent one like so:
...
These are listed in the [`ducklake_schema` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_schema).
...
- `SNAPSHOT_ID` is a `BIGINT` referring to the `snapshot_id` column in the [`ducklake_snapshot` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot)

### List Tables

We can list the tables available in a schema for a specific snapshot using the [`ducklake_table` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table):
...
- `SCHEMA_ID` is a `BIGINT` referring to the `schema_id` column in the [`ducklake_schema` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_schema)
- `SNAPSHOT_ID` is a `BIGINT` referring to the `snapshot_id` column in the [`ducklake_snapshot` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot)

### Show the Structure of a Table

For each given table, we can list the available top-level columns using the [`ducklake_column` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_column):
...
- `TABLE_ID` is a `BIGINT` referring to the `table_id` column in the [`ducklake_table` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table)
- `SNAPSHOT_ID` is a `BIGINT` referring to the `snapshot_id` column in the [`ducklake_snapshot` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot)

> Note that DuckLake supports nested columns – the filter for `parent_column IS NULL` only shows the top-level columns.

For the list of supported data types, please refer to the [“Types” page]({% link docs/stable/specification/catalog/types.md %}).
...
> [`ducklake_data_file`]({% link docs/stable/specification/catalog/tables.md %}#ducklake_data_file) and
> [`ducklake_delete_file`]({% link docs/stable/specification/catalog/tables.md %}#ducklake_delete_file)
> entries (`path_is_relative`) to the `data_path` prefix from
> [`ducklake_metadata`]({% link docs/stable/specification/catalog/tables.md %}#ducklake_metadata).
...
The [`ducklake_file_column_statistics` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_file_column_statistics) contains the file-level statistics.
...
- `TABLE_ID` is a `BIGINT` referring to the `table_id` column in the [`ducklake_table` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table).
- `COLUMN_ID` is a `BIGINT` referring to the `column_id` column in the [`ducklake_column` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_column).
...
- create a new snapshot in [`ducklake_snapshot`]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot) and
- log the changes a snapshot made in [`ducklake_snapshot_changes`]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot_changes)
...
- `CHANGES` contains a list of changes performed by the snapshot. See the list of possible values in the [`ducklake_snapshot_changes` table's documentation]({% link docs/stable/specification/catalog/tables.md %}#ducklake_snapshot_changes).

### `CREATE SCHEMA`

A schema is a collection of tables. In order to create a new schema, we can just insert into the [`ducklake_schema` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_schema):
...
Creating a table in a schema is very similar to creating a schema. We insert into the [`ducklake_table` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table):
...
- `SNAPSHOT_ID` is the snapshot identifier of the new snapshot as described above.
- `SCHEMA_ID` is a `BIGINT` referring to the `schema_id` column in the [`ducklake_schema` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_schema) table.
- `TABLE_NAME` is just the name of the new table.

A table needs some columns, we can add columns to the new table by inserting into the [`ducklake_column` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_column) table.
...
- `COLUMN_TYPE` is the data type of the column. See the [“Types” page]({% link docs/stable/specification/catalog/types.md %}) for details.
...
- `ROW_ID_START` is the first logical row ID from the file. This number can be read from the [`ducklake_table_stats` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table_stats) via column `next_row_id`.

> We have omitted some complexity around relative paths, encrypted files, partitioning and partial files in this example.
> Refer to the [`ducklake_data_file` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_data_file) documentation for details.


> DuckLake also supports changing the schema, see [schema evolution]({% link docs/stable/duckdb/getting_started/schema_evolution.md %}).

We will also have to update some statistics in the [`ducklake_table_stats` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table_stats) and [`ducklake_table_column_stats` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table_column_stats)` tables.
...
- `TABLE_ID` is a `BIGINT` referring to the `table_id` column in the [`ducklake_table` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_table).
- `COLUMN_ID` is a `BIGINT` referring to the `column_id` column in the [`ducklake_column` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_column).
- `DATA_FILE_ID` is a `BIGINT` referring to the `data_file_id` column in the [`ducklake_data_file` table]({% link docs/stable/specification/catalog/tables.md %}#ducklake_data_file).
